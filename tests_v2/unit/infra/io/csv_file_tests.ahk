...
;   - Separator: ';'
;   - Encoding: UTF-8
;   - Header without quotes: col1;col2;col3
;   - Data rows quoted: "v1";"v2";"v3"
;   - Internal quotes escaped as doubled quotes: "he said ""hi"""
;   - LF (`n) at the end of every line
;
; Naming: `csvInst` instead of `csvFile` to avoid colliding with the class.
...
class CsvFileTests extends TestCase
{
    ...
    ensure_header_returns_false_when_file_exists()
    {
        ...
        Assert.False(csvInst.EnsureHeader(["a", "b"]),
            "Second call should detect existing file")
    }
    ...
    append_row_escapes_internal_quotes_as_double_quotes()
    {
        ...
        csvInst.AppendRow(['he said "hi"'])

        content := Fixtures.FileReadAll(path)
        Assert.Contains('"he said ""hi"""', content)
    }
    ...
    {
        path := Fixtures.TempPath("csv")
        csvInst := CsvFile(path)   ; expected=0 -> no validation
    ...
    write_all_rows_overwrites_existing_file()
    {
        ...
        Assert.False(InStr(content, "old;header") > 0,
            "Old content should have been removed")
    }
    ...
    write_all_rows_validates_rows_before_touching_disk()
    {
        ...
        ; Second row has 2 columns instead of 3
        ...
        Assert.False(FileExist(path),
            "Validation failed -> file should not exist")
    }
    ...
    read_all_rows_skips_header()
    {
        ...
        Assert.Equal(2, rows.Length, "Header must not appear")
    }
    ...
    {
        path := Fixtures.TempPath("csv")
        ; Manual write: header + empty line + row + empty line
        ...
        csvInst := CsvFile(path)
        rows := csvInst.ReadAllRows()
        Assert.Equal(1, rows.Length, "Empty lines must be skipped")
    }
    ...
    read_all_rows_skips_rows_with_wrong_column_count_when_expected()
    {
        ...
        ; Manually create a row with fewer columns (no validation on append)
        FileAppend("c1;c2;c3`n" '"a";"b";"c"' "`n" '"x";"y"' "`n", path, "UTF-8")

        rows := csvInst.ReadAllRows()
        Assert.Equal(1, rows.Length, "Row with fewer columns than expected must be skipped")
    }
    ...
    escape_field_doubles_internal_quotes()
    {
        result := CsvFile.EscapeField('he said "hi"')
